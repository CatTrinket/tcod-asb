...
as $$
    select case when not unova then
        case when (stage, length) = (2, 2) then
            ...
            when level between 36 and 42 then 4
        ...
        when (stage, length) = (2, 3) then
            ...
            when level between 30 and 43 then 4
            end
...
changes (id, exp) as (
    select pf.id, exp(ps.level, s.stage, fl.length,
                      false, ps.id between 494 and 649)
    ...
    where ps.level is not null
)

update evolution_methods
set experience = (select exp from changes where id=evolved_form_id)
where evolved_form_id in (select id from changes)
returning evolved_form_id, experience;
...
-- 3 exp.
update evolution_methods set experience = 3
where evolved_form_id in (
